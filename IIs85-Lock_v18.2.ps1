...
try
{
    ...
    $mime = Get-WebConfigurationProperty -filter "//staticContent/mimeMap" -Name fileExtension | Where-Object{$_.value -eq '.bat' -or $_.value -eq '.com' -or $_.value -eq '.csh' -or $_.value -eq '.dll' -or $_.value -eq '.exe'}
    ...
    If ($reg -ne $nul)
     ...
     If (!(Get-WebConfigurationProperty -filter system.web/authorization -pspath MACHINE/WEBROOT -name collection[roles="Administrators"]))
     {
     Add-WebConfiguration -Filter /system.web/authorization -PSPath "MACHINE/WEBROOT" -Value @{accesstype="Allow"; roles="Administrators"} -ea stop
     }
     Clear-WebConfiguration -Filter "/system.web/authorization/allow[@users='*' and @roles='' and @verbs='']" -PSPath "MACHINE/WEBROOT" -ea stop
     # may want to add a check that users exists before attempting the clear.
...
IISW-SI-000250/ V-76863  IISW-SI-000260/ V-76883
IISW-SI-000261/ V-76885  IISW-SI-000262/ V-76887
IISW-SI-000263/ V-76889  IISW-SI-000264/ V-76891
#>
     ...
     write-log -msg "Configuring for IISW-SI-000203,242,243,249,250 / V-76779,V-76851,V-76853,V-76861,V-76863 This is open finding as it should be 'SslRequireCert but that setting breaks most management websites within System Center Products"
    ...
    # Set-WebConfigurationProperty -pspath 'machine/webroot/apphost' -filter "system.applicationhost/sites/sitedefaults/logfile" -name "logtargetW3C" -value "File,ETW"
     ...
     $webDAV = Get-WindowsFeature | Where-Object {$_.name -eq "web-DAV-Publishing"}
    ...
    write-log -msg "Configuring for IISW-SI-000245-246 / V-76857,V-76859"
...
write-log -msg "Configuring for IISW-SI-000225-229,230,233-235,242-246, 249-250 / V-76817,V-76819,V-76821,V-76823,V-76825,V-76829,V-76835,V-76837,V-76839,V-76851,V-76853V-76855,V-76857,V-76859,V-76861,V-76863"
